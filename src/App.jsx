...
function Calendario({ area, color, tipoLabel = ["Inspección", "Proyecto"] }) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 16 }}>
      ...
      >
        {/* Cabecera de días */}
        <div style={{ display: "grid", gridTemplateColumns: "64px repeat(7,1fr)", borderBottom: `1px solid ${T.line}`, paddingBottom: 2 }}>
          <div />
          {days.map((d) => {
            const isToday = isoDate(d) === todayISO();
            return (
              <div key={d.toISOString()} style={{ textAlign: "center", padding: "6px 4px 14px", background: isToday ? `${color}0D` : "transparent", borderRadius: "10px 10px 0 0" }}>
                <div style={{ fontSize: 11, color: T.inkSoft, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.8 }}>
                  {d.toLocaleDateString("es-CR", { weekday: "short" })}
                </div>
                <div style={{
                  fontSize: 17, fontWeight: 800, color: isToday ? "#fff" : T.ink,
                  background: isToday ? color : "transparent", width: 32, height: 32, lineHeight: "32px",
                  borderRadius: "50%", margin: "6px auto 0", boxShadow: isToday ? `0 3px 8px ${color}4D` : "none",
                }}>{d.getDate()}</div>
                <div style={{ display: "flex", justifyContent: "center", gap: 3, marginTop: 6, minHeight: 5 }}>
                  {rangosDelDia(d).map((r) => (
                    <div key={r.od} title={`OD ${r.od}: ${r.min} a ${r.max}`} style={{ width: 18, height: 5, borderRadius: 3, background: r.color }} />
                  ))}
                </div>
              </div>
            );
        ...
        {/* Grilla clara y espaciosa, con líneas suaves por hora */}
        <div style={{ display: "grid", gridTemplateColumns: "64px repeat(7,1fr)", position: "relative" }}>
          <div>
            {hours.map((h) => (
              <div key={h} style={{ height: HOUR_PX, fontSize: 11.5, color: T.gray, fontWeight: 600, textAlign: "right", paddingRight: 12, transform: "translateY(-7px)" }}>
                {String(h).padStart(2, "0")}:00
              </div>
            ))}
          </div>

          {/* columnas de días con líneas suaves horizontales */}
          {days.map((d, di) => {
            const isToday = isoDate(d) === todayISO();
            ...
            return (
              <div key={d.toISOString()} style={{
                ...
                background: rangosHoy.length > 0 ? `${rangosHoy[0].color}10` : isToday ? `${color}0A` : "#fff",
              }}>
                {hours.map((h) => (
                  <div key={h} style={{ height: HOUR_PX, borderBottom: `1px solid ${T.line}` }} />
                ))}
                {isToday && showNowLine && (
                  <div style={{
                    ...
                    <div style={{ width: 8, height: 8, borderRadius: "50%", background: T.accent, position: "absolute", left: -4, top: -4, boxShadow: `0 0 0 3px ${T.accent}33` }} />
                  </div>
                )}
                {eventosDelDia(d).map((e) => (
                  <div
                    ...
                    style={{
                      position: "absolute", left: 4, right: 4, top: topFor(e.hora), minHeight: 54,
                      background: e.tipo === tipoLabel[1] ? T.greenSoft : T.accentSoft,
                      borderLeft: `4px solid ${e.tipo === tipoLabel[1] ? T.green : T.accent}`,
                      color: e.tipo === tipoLabel[1] ? T.green : T.accent,
                      borderRadius: 8, padding: "6px 8px", fontSize: 11.5, fontWeight: 700,
                      overflow: "hidden", boxShadow: "0 2px 6px rgba(16,24,38,.08)", zIndex: 1,
                    }}
                  >
                    <div style={{ fontSize: 12.5 }}>{e.hora} · {e.od}</div>
                    <div style={{ fontWeight: 500, opacity: 0.85, fontSize: 11, marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{e.personas}</div>
                  </div>
                ))}
              </div>
            );
          ...
          {eventosSemana.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4, maxHeight: 280, overflowY: "auto" }}>
              {eventosSemana.map((e, i) => (
                <div key={e.id} style={{ display: "flex", gap: 10, alignItems: "flex-start", padding: "10px 8px", borderRadius: 8, borderBottom: i < eventosSemana.length - 1 ? `1px solid ${T.line}` : "none" }}>
                  <div style={{ marginTop: 5, width: 9, height: 9, borderRadius: "50%", flexShrink: 0, background: e.tipo === tipoLabel[1] ? T.green : T.accent }} />

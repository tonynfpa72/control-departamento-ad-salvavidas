...
function RankingEntrenamiento() {
  ...
  const reiniciarPersona = async (codigo) => {
    const { error } = await supabase.from("entrenamiento_puntajes").delete().eq("personal_codigo", codigo);
    const { data } = await supabase.from("entrenamiento_puntajes").select("*");
    const siguenQuedando = (data || []).some((p) => p.personal_codigo === codigo);
    if (error || siguenQuedando) {
      alert(
        "No se pudo reiniciar a esta persona.\n\n" +
        (error ? "Error de Supabase: " + error.message : "Supabase no borró ninguna fila (0 afectadas).") +
        "\n\nCausa más probable: a la tabla entrenamiento_puntajes le falta la política de borrado (DELETE) en Row Level Security. " +
        "Corre el script SQL de arreglo que te compartí para agregarla."
      );
      setConfirmando(null);
      return;
    }
...
const MAX_PUNTOS_SEGMENTO = {
  compuertas: 160, tablas_verdad: 120, escalera: 50, simplex: 80, notifier: 0, nfpa72: 200, electronica: 190, nicet: 500,
};
...
function Entrenamiento() {
  ...
  const reiniciarSegmento = async (segmentoId, borrarRanking) => {
    if (!jugador) return;
    if (borrarRanking) {
      const { error } = await supabase.from("entrenamiento_puntajes").delete().eq("personal_codigo", jugador.codigo).eq("segmento", segmentoId);
      const { data } = await supabase.from("entrenamiento_puntajes").select("*").eq("personal_codigo", jugador.codigo);
      const siguenQuedando = (data || []).some((p) => p.segmento === segmentoId);
      if (error || siguenQuedando) {
        alert(
          "No se pudo borrar el ranking de este módulo.\n\n" +
          (error ? "Error de Supabase: " + error.message : "Supabase no borró ninguna fila (0 afectadas).") +
          "\n\nCausa más probable: a la tabla entrenamiento_puntajes le falta la política de borrado (DELETE) en Row Level Security."
        );
        return;
      }
      const { data: todos } = await supabase.from("entrenamiento_puntajes").select("*");
      if (todos) setPuntajes(todos);
    }
  };
...
// Módulo NICET (Fire Protection — Fire Alarm Systems): 50 preguntas
// reales de práctica (traducidas al español), con selector de idioma
// EN/ES y soporte para preguntas de selección múltiple ("elige dos/
// tres"). La insignia NICET (con la imagen real del sello) se gana con
// 85% o más.
const NICET_FIG_1498 = "data:image/jpeg;base64,";
const NICET_FIG_1499 = "data:image/jpeg;base64,";
const NICET_FIG_509 = "data:image/jpeg;base64,";
const NICET_FIG_R1R4 = "data:image/jpeg;base64,";
const NICET_FIG_500 = "data:image/jpeg;base64,";
const NICET_FIG_1112 = "data:image/jpeg;base64,";
const NICET_FIG_CLASS = "data:image/jpeg;base64,";
const NICET_FIG_1500 = "data:image/jpeg;base64,";
const NICET_FIG_1109 = "data:image/jpeg;base64,";
const NICET_FIG_1108 = "data:image/jpeg;base64,";
const NICET_FIG_1111 = "data:image/jpeg;base64,";

const PREGUNTAS_NICET = [
  { en: { texto: "What is the difference between a preliminary plan and a working plan?", opciones: ["Detail contained therein", "Government approval", "Inspector's approval", "Number of fire detectors"] },
    es: { texto: "¿Cuál es la diferencia entre un plano preliminar y un plano de trabajo (working plan)?", opciones: ["El nivel de detalle que contienen", "Aprobación gubernamental", "Aprobación del inspector", "Cantidad de detectores de incendio"] },
    correcta: 0 },
  { en: { texto: "While standing on the floor, using a battery-operated drill to cut holes overhead, a worker should wear which of the two of the following pieces of safety equipment? (Choose two)", opciones: ["Hard hat", "Harness", "Rubber-soled boots", "Safety glasses"] },
    es: { texto: "Estando de pie en el piso, usando un taladro a batería para hacer huecos hacia arriba, ¿cuáles 2 equipos de seguridad debe usar el trabajador? (Elige dos)", opciones: ["Casco", "Arnés", "Botas con suela de hule", "Gafas de seguridad"] },
    multiple: true, correctas: [0, 3] },
  { en: { texto: "A hallway is 10ft wide and 120ft long with a smooth, flat, 10ft ceiling. Per NFPA 72, how many photoelectric spot type smoke detectors will be needed to protect the hallway?", opciones: ["3", "4", "5", "6"] },
    es: { texto: "Un pasillo mide 10ft de ancho y 120ft de largo, con techo liso y plano de 10ft de altura. Según NFPA 72, ¿cuántos detectores de humo fotoeléctricos tipo spot se necesitan para proteger el pasillo?", opciones: ["3", "4", "5", "6"] },
    correcta: 1 },
  { en: { texto: "When a protected premises has a 10.9ft joisted ceiling in a 120ft by 50ft space, how many rate of rise heat detectors listed with a 50ft spacing shall be required?", opciones: ["3", "4", "6", "8"] },
    es: { texto: "Cuando un local protegido tiene un techo con vigas (joisted) de 10.9ft en un espacio de 120ft por 50ft, ¿cuántos detectores de calor de razón de aumento (rate of rise) con espaciado listado de 50ft se requieren?", opciones: ["3", "4", "6", "8"] },
    imagen: NICET_FIG_1498, correcta: 2 },
  { en: { texto: "Which two of the following fire alarm circuits will require an end-of-line resistor? (Choose two)", opciones: ["Bell", "Flow switch", "Signaling line", "Strobe"] },
    es: { texto: "¿Cuáles 2 de los siguientes circuitos de alarma contra incendio requieren una resistencia de fin de línea (EOL)? (Elige dos)", opciones: ["Timbre", "Interruptor de flujo", "Línea de señalización", "Estrobo"] },
    multiple: true, correctas: [1, 2] },
  { en: { texto: "On the jobsite, a worker encounters a puddle of liquid that has a chemical odor. Which of the following steps should be performed first?", opciones: ["Call OSHA authorities", "Notify central station", "Notify a supervisor", "Use a nearby absorbent materials to scrub the spill"] },
    es: { texto: "En el sitio de trabajo, un trabajador encuentra un charco de líquido con olor químico. ¿Cuál de los siguientes pasos debe realizarse primero?", opciones: ["Llamar a las autoridades de OSHA", "Notificar a la central de monitoreo", "Notificar a un supervisor", "Usar materiales absorbentes cercanos para limpiar el derrame"] },
    correcta: 2 },
  { en: { texto: "An 180 foot long corridor is 10 feet wide, with a smooth 10 foot high ceiling, interrupted by 4 inch beams spaced every 10 feet. How many heat detectors with a listed 30 foot spacing are required?", opciones: ["4", "5", "6", "7"] },
    es: { texto: "Un corredor de 180ft de largo mide 10ft de ancho, con techo liso de 10ft de altura, interrumpido por vigas de 4 pulgadas cada 10ft. ¿Cuántos detectores de calor con espaciado listado de 30ft se requieren?", opciones: ["4", "5", "6", "7"] },
    correcta: 2 },
  { en: { texto: "A room with a 10ft high ceiling measures 60ft x 60ft. The beams are 8 inches deep and 3ft apart. How many smoke detectors are necessary to properly protect this space?", opciones: ["8", "9", "11", "12"] },
    es: { texto: "Un cuarto con techo de 10ft de altura mide 60ft x 60ft. Las vigas tienen 8 pulgadas de profundidad y están separadas 3ft. ¿Cuántos detectores de humo se necesitan para proteger correctamente este espacio?", opciones: ["8", "9", "11", "12"] },
    imagen: NICET_FIG_1499, correcta: 2 },
  { en: { texto: "After the fire alarm system installation has been completed, at what frequency should system smoke detectors undergo functional testing, after the alarm contractor has performed the acceptance test of the fire alarm system?", opciones: ["Weekly", "Monthly", "Semi-annually", "Annually"] },
    es: { texto: "Después de completada la instalación del sistema de alarma, y tras la prueba de aceptación realizada por el contratista, ¿con qué frecuencia deben someterse a prueba funcional los detectores de humo del sistema?", opciones: ["Semanal", "Mensual", "Semestral", "Anual"] },
    correcta: 3 },
  { en: { texto: "What part requires special handling for disposal?", opciones: ["Fire alarm control panel", "Heat detector", "Lead acid batteries", "Photo smoke detector"] },
    es: { texto: "¿Qué componente requiere manejo especial para su desecho?", opciones: ["Panel de control de alarma", "Detector de calor", "Baterías de plomo-ácido", "Detector de humo fotoeléctrico"] },
    correcta: 2 },
  { en: { texto: "What is the maximum number of 16 gauge TFN wires that can be installed in a ¾ in (19mm) liquid tight flexible metal conduit?", opciones: ["26", "30", "37", "39"] },
    es: { texto: "¿Cuál es el número máximo de cables TFN calibre 16 que se pueden instalar en un conduit metálico flexible tipo líquido de ¾ in (19mm)?", opciones: ["26", "30", "37", "39"] },
    correcta: 1 },
  { en: { texto: "Which one of the following shall be tested semi-annually?", opciones: ["Audible device", "Duct detector", "Remote annunciator", "Supervisory switch"] },
    es: { texto: "¿Cuál de los siguientes debe probarse semestralmente?", opciones: ["Dispositivo audible", "Detector de ducto", "Anunciador remoto", "Interruptor de supervisión"] },
    correcta: 3 },
  { en: { texto: "According to figure 1500, how many notification appliances are shown?", opciones: ["4", "5", "8", "12"] },
    es: { texto: "Según la figura 1500, ¿cuántos dispositivos de notificación se muestran?", opciones: ["4", "5", "8", "12"] },
    imagen: NICET_FIG_1500, correcta: 1 },
  { en: { texto: "In figure 509, what is the distance (l) measured on the slope of the roof?", opciones: ["24' 8-1/4\"", "25' 1-7/8\"", "25' 3-13/16\"", "27'"] },
    es: { texto: "En la figura 509, ¿cuál es la distancia (l) medida sobre la pendiente del techo?", opciones: ["24' 8-1/4\"", "25' 1-7/8\"", "25' 3-13/16\"", "27'"] },
    imagen: NICET_FIG_509, correcta: 2 },
  { en: { texto: "Prior to testing the system, the technician must ensure that all system software stored in volatile memory is protected from which three of the following? (Choose three)", opciones: ["Corruption", "Spikes", "Static discharge", "Voiding system warranty"] },
    es: { texto: "Antes de probar el sistema, el técnico debe asegurarse de que todo el software almacenado en memoria volátil esté protegido contra ¿cuáles 3 de los siguientes? (Elige tres)", opciones: ["Corrupción", "Picos de voltaje", "Descarga estática", "Anular la garantía del sistema"] },
    multiple: true, correctas: [0, 1, 2] },
  { en: { texto: "Which plans include fire detection system details?", opciones: ["Electrical", "Utility", "Architectural", "Site"] },
    es: { texto: "¿Qué planos incluyen los detalles del sistema de detección de incendios?", opciones: ["Eléctricos", "De servicios públicos", "Arquitectónicos", "De sitio"] },
    correcta: 0 },
  { en: { texto: "What are the voltage drops across R4, R3, R2 and R1 resistors?", opciones: ["1.5,1.5,1.5,1.5", "1.5,1.5,3.0,1.5", "1.5,1.5,3.0,3.0", "3.0,1.5,1.5,1.5"] },
    es: { texto: "¿Cuáles son las caídas de voltaje en las resistencias R4, R3, R2 y R1?", opciones: ["1.5,1.5,1.5,1.5", "1.5,1.5,3.0,1.5", "1.5,1.5,3.0,3.0", "3.0,1.5,1.5,1.5"] },
    imagen: NICET_FIG_R1R4, correcta: 3 },
  { en: { texto: "What does the symbol shown in Figure 500 represent?", opciones: ["Audible notification appliance", "Manual station", "Motion detector", "Smoke detector"] },
    es: { texto: "¿Qué representa el símbolo mostrado en la Figura 500?", opciones: ["Dispositivo de notificación audible", "Estación manual", "Detector de movimiento", "Detector de humo"] },
    imagen: NICET_FIG_500, correcta: 0 },
  { en: { texto: "A 24vdc siren, which draws 1 amp, is located 500ft (152.4m) from the fire alarm control panel. The siren will operate with a maximum of a 10% drop in voltage. At a normal temperature of 70°F (21°C), what is the smallest size copper wire which can be used?", opciones: ["12 AWG", "14 AWG", "16 AWG", "18 AWG"] },
    es: { texto: "Una sirena de 24VDC que consume 1 amperio está ubicada a 500ft (152.4m) del panel de control de alarma. La sirena operará con una caída máxima de voltaje del 10%. A una temperatura normal de 70°F (21°C), ¿cuál es el calibre de cable de cobre más pequeño que se puede usar?", opciones: ["12 AWG", "14 AWG", "16 AWG", "18 AWG"] },
    correcta: 0 },
  { en: { texto: "When is it acceptable for a worker to use an ungrounded power tool?", opciones: ["The operator is outdoors, and their body will remain in contact with the earth", "The tool is being powered by a generator", "The tool is connected to a GFCI extension cord", "The tool is double insulated"] },
    es: { texto: "¿Cuándo es aceptable que un trabajador use una herramienta eléctrica sin conexión a tierra?", opciones: ["El operador está afuera y su cuerpo permanece en contacto con la tierra", "La herramienta es alimentada por un generador", "La herramienta está conectada a una extensión con GFCI", "La herramienta tiene doble aislamiento"] },
    correcta: 3 },
  { en: { texto: "The requirement forbidding fire alarm circuits from being \"T\" tapped, unless indicated by the manufacturer's instructions, is intended to prevent which type of improper function?", opciones: ["Annunciation", "Monitoring integrity of installation conductors", "Power consumption", "Voltage"] },
    es: { texto: "El requisito que prohíbe hacer derivaciones en \"T\" en los circuitos de alarma contra incendio, salvo que el fabricante lo indique, busca evitar ¿qué tipo de función incorrecta?", opciones: ["Anunciación", "Supervisión de la integridad de los conductores de instalación", "Consumo de energía", "Voltaje"] },
    correcta: 1 },
  { en: { texto: "What type of line illustrates canopies, sheds and other open walled structures on drawings?", opciones: ["Broken", "Dotted", "Double", "Single"] },
    es: { texto: "¿Qué tipo de línea representa marquesinas, cobertizos y otras estructuras abiertas sin paredes en los planos?", opciones: ["Discontinua", "Punteada", "Doble", "Sencilla"] },
    correcta: 0 },
  { en: { texto: "Unless otherwise recommended by the equipment manufacturer (UL 217), single and multiple station smoke alarms shall remain in service for what maximum time period?", opciones: ["10 years from date of first test", "10 years from date of installation", "10 years from date of manufacture", "10 years from date of re-calibration"] },
    es: { texto: "A menos que el fabricante del equipo (UL 217) recomiende lo contrario, ¿por cuánto tiempo máximo deben permanecer en servicio las alarmas de humo de una y varias estaciones?", opciones: ["10 años desde la primera prueba", "10 años desde la instalación", "10 años desde la fabricación", "10 años desde la recalibración"] },
    correcta: 2 },
  { en: { texto: "What is the limit to the number of quarter bends allowed in a run of electrical metallic tubing between outlet and outlet, fitting and fitting, or outlet and fitting?", opciones: ["2", "3", "4", "5"] },
    es: { texto: "¿Cuál es el límite de codos de un cuarto de vuelta permitidos en un tramo de tubería metálica eléctrica (EMT) entre salida y salida, accesorio y accesorio, o salida y accesorio?", opciones: ["2", "3", "4", "5"] },
    correcta: 2 },
  { en: { texto: "What is the maximum operating temperature of TW insulation?", opciones: ["140°F (60°C)", "167°F (75°C)", "194°F (90°C)", "392°F (200°C)"] },
    es: { texto: "¿Cuál es la temperatura máxima de operación del aislamiento tipo TW?", opciones: ["140°F (60°C)", "167°F (75°C)", "194°F (90°C)", "392°F (200°C)"] },
    correcta: 0 },
  { en: { texto: "If a fire alarm system has defects that cannot be corrected during system maintenance, the technician is required to take which one of the following actions?", opciones: ["Inform the building owner before leaving the premises", "Inform the building owner in writing within 24 hours", "Inform the building owner within 48 hours", "Inform the building maintenance supervisor within 7 days"] },
    es: { texto: "Si un sistema de alarma contra incendio tiene defectos que no pueden corregirse durante el mantenimiento, ¿qué acción debe tomar el técnico?", opciones: ["Informar al dueño del edificio antes de salir del lugar", "Informar por escrito al dueño en 24 horas", "Informar al dueño en 48 horas", "Informar al supervisor de mantenimiento en 7 días"] },
    correcta: 0 },
  { en: { texto: "Household fire alarm systems shall be tested by a qualified service technician at which maximum interval?", opciones: ["1 year", "2 years", "3 years", "4 years"] },
    es: { texto: "¿Con qué intervalo máximo deben probarse los sistemas de alarma contra incendio residenciales por un técnico de servicio calificado?", opciones: ["1 año", "2 años", "3 años", "4 años"] },
    correcta: 0 },
  { en: { texto: "What should be done to the originals of the contract documents in order to complete as-built drawings?", opciones: ["Add latest field cross sections by cross hatch to profile sections and plan view", "Compile all as-built field data and change the original drawings accordingly", "Submit documents to Architect for approval", "Turn all documents over to the building's owner"] },
    es: { texto: "¿Qué se debe hacer con los originales de los documentos contractuales para completar los planos \"as-built\"?", opciones: ["Agregar las últimas secciones transversales de campo con sombreado", "Recopilar todos los datos de campo y modificar los planos originales en consecuencia", "Enviar los documentos al arquitecto para aprobación", "Entregar todos los documentos al dueño del edificio"] },
    correcta: 1 },
  { en: { texto: "Per NFPA 72, how many fixed temperature heat detectors with a listed spacing of 30ft would be required in a rectangular space 75ft long, 50ft wide and with a smooth, flat ceiling 10ft high?", opciones: ["4", "5", "6", "8"] },
    es: { texto: "Según NFPA 72, ¿cuántos detectores de calor de temperatura fija con espaciado listado de 30ft se requieren en un espacio rectangular de 75ft de largo, 50ft de ancho, con techo liso y plano de 10ft de altura?", opciones: ["4", "5", "6", "8"] },
    correcta: 2 },
  { en: { texto: "Which one of the following types of gloves shall be worn when working with raceway systems?", opciones: ["Cotton", "Metal-mesh", "Polyester", "Rubber"] },
    es: { texto: "¿Qué tipo de guantes se deben usar al trabajar con sistemas de raceway (tubería/canalización)?", opciones: ["Algodón", "Malla metálica", "Poliéster", "Hule"] },
    correcta: 3 },
  { en: { texto: "Which three of the following conditions are symptoms of a heart attack? (Choose three)", opciones: ["Heart burn", "Memory loss", "Pain in the chest", "Shortness of breath"] },
    es: { texto: "¿Cuáles 3 de las siguientes condiciones son síntomas de un ataque al corazón? (Elige tres)", opciones: ["Acidez", "Pérdida de memoria", "Dolor en el pecho", "Falta de aire"] },
    multiple: true, correctas: [0, 2, 3] },
  { en: { texto: "Per NFPA 72, the initial 100% acceptance test of each new fire alarm system includes which three of the following elements? (Choose three)", opciones: ["Ground faults", "Initiating devices", "Loop resistances", "Smoke detector sensitivity"] },
    es: { texto: "Según NFPA 72, la prueba inicial de aceptación al 100% de cada sistema nuevo incluye ¿cuáles 3 de los siguientes elementos? (Elige tres)", opciones: ["Fallas a tierra", "Dispositivos iniciadores", "Resistencias de lazo", "Sensibilidad de detectores de humo"] },
    multiple: true, correctas: [0, 1, 2] },
  { en: { texto: "Three restorable fixed-temperature detectors are connected to an initiating circuit on a conventional fire alarm system. At a minimum, when should these detectors be replaced?", opciones: ["1 year", "7 years", "10 years", "15 years"] },
    es: { texto: "Tres detectores restaurables de temperatura fija están conectados a un circuito iniciador en un sistema convencional de alarma. Como mínimo, ¿cuándo deben reemplazarse estos detectores?", opciones: ["1 año", "7 años", "10 años", "15 años"] },
    correcta: 2 },
  { en: { texto: "A crew member has a suspected leg fracture. Before moving the victim, what is the generally recommended first step in treating the injury?", opciones: ["Apply ice to the area over the fracture", "Apply pressure to the wound", "Immobilize the leg", "Sterilize the area around the suspected fracture"] },
    es: { texto: "Un miembro del equipo tiene una posible fractura de pierna. Antes de mover a la víctima, ¿cuál es el primer paso generalmente recomendado para tratar la lesión?", opciones: ["Aplicar hielo sobre la fractura", "Aplicar presión sobre la herida", "Inmovilizar la pierna", "Esterilizar el área alrededor de la fractura"] },
    correcta: 2 },
  { en: { texto: "What standard covers the installation of audible and visible notification appliances?", opciones: ["NFPA 90A", "NFPA 72 2007, Ch 6", "NFPA 72 2007, Ch 7", "NFPA 72 2007, Ch 9 & 10"] },
    es: { texto: "¿Qué norma cubre la instalación de dispositivos de notificación audibles y visuales?", opciones: ["NFPA 90A", "NFPA 72 2007, Cap 6", "NFPA 72 2007, Cap 7", "NFPA 72 2007, Cap 9 y 10"] },
    correcta: 2 },
  { en: { texto: "At minimum, when working near power lines with voltage of 600v and lower, an aerial lift operator shall maintain which distance from the line?", opciones: ["3ft (0.9m)", "5ft (1.5m)", "6ft (1.8m)", "10ft (3.0m)"] },
    es: { texto: "Como mínimo, al trabajar cerca de líneas eléctricas de 600V o menos, ¿qué distancia debe mantener el operador de un elevador aéreo respecto a la línea?", opciones: ["3ft (0.9m)", "5ft (1.5m)", "6ft (1.8m)", "10ft (3.0m)"] },
    correcta: 3 },
  { en: { texto: "Which type of detector is most suitable for protecting atriums and similar places?", opciones: ["Ionization", "Linear beam", "Rate compensated", "Spot type"] },
    es: { texto: "¿Qué tipo de detector es más adecuado para proteger atrios y espacios similares?", opciones: ["Ionización", "Haz lineal", "Compensado por razón", "Tipo puntual"] },
    correcta: 1 },
  { en: { texto: "Which three circuits are equivalent to each other? (Choose three)", opciones: ["A", "B", "C", "D"] },
    es: { texto: "¿Cuáles 3 circuitos son equivalentes entre sí? (Elige tres)", opciones: ["A", "B", "C", "D"] },
    imagen: NICET_FIG_1112, multiple: true, correctas: [0, 1, 2] },
  { en: { texto: "An octagonal room with a smooth ceiling measures 60 feet wide, 60 feet long, and each side of the octagon measures 25 feet. Using a listed detector spacing of 50ft, how many heat detectors are needed?", opciones: ["3", "4", "6", "7"] },
    es: { texto: "Un cuarto octagonal con techo liso mide 60 pies de ancho, 60 pies de largo, y cada lado del octágono mide 25 pies. Usando un espaciado de detector listado de 50ft, ¿cuántos detectores de calor se necesitan?", opciones: ["3", "4", "6", "7"] },
    correcta: 1 },
  { en: { texto: "In the drawing, the shown circuit belongs to which class?", opciones: ["A", "B", "X", "Y"] },
    es: { texto: "En el dibujo, ¿a qué clase pertenece el circuito mostrado?", opciones: ["A", "B", "X", "Y"] },
    imagen: NICET_FIG_CLASS, correcta: 1 },
  { en: { texto: "According to figure 1500, how many devices are shown in total?", opciones: ["7", "12", "15", "20"] },
    es: { texto: "Según la figura 1500, ¿cuántos dispositivos se muestran en total?", opciones: ["7", "12", "15", "20"] },
    imagen: NICET_FIG_1500, correcta: 2 },
  { en: { texto: "When using a corded power tool, where is the required location for the ground fault circuit interrupter breaker?", opciones: ["In the cable of the power tool", "In the electrical panel", "In the service vehicle", "Within the power tool"] },
    es: { texto: "Al usar una herramienta eléctrica con cable, ¿dónde debe ubicarse el interruptor de falla a tierra (GFCI) requerido?", opciones: ["En el cable de la herramienta", "En el panel eléctrico", "En el vehículo de servicio", "Dentro de la herramienta"] },
    correcta: 1 },
  { en: { texto: "What is the current on the primary side of the transformer?", opciones: ["1 amp", "2 amps", "3 amps", "5 amps"] },
    es: { texto: "¿Cuál es la corriente en el lado primario del transformador?", opciones: ["1 amp", "2 amps", "3 amps", "5 amps"] },
    imagen: NICET_FIG_1109, correcta: 1 },
  { en: { texto: "With the diodes in the current configuration, which lamp is lit?", opciones: ["L1", "L2", "Both L1 and L2", "Neither"] },
    es: { texto: "Con los diodos en la configuración actual, ¿qué lámpara enciende?", opciones: ["L1", "L2", "Ambas L1 y L2", "Ninguna"] },
    imagen: NICET_FIG_1108, correcta: 1 },
  { en: { texto: "Which relays will operate?", opciones: ["K1", "K2", "Both K1 and K2", "Neither K1 nor K2"] },
    es: { texto: "¿Qué relés operarán?", opciones: ["K1", "K2", "Ambos K1 y K2", "Ninguno"] },
    imagen: NICET_FIG_1111, correcta: 0 },
  { en: { texto: "What is the maximum number of 14 gauge wire which can be pulled straight through a 4\" (102mm) 2-1/8\" (54mm) square trade size metal electrical box?", opciones: ["13", "15", "17", "20"] },
    es: { texto: "¿Cuál es el número máximo de cable calibre 14 que se puede pasar recto a través de una caja eléctrica metálica cuadrada de 4\" (102mm) x 2-1/8\" (54mm)?", opciones: ["13", "15", "17", "20"] },
    correcta: 1 },
  { en: { texto: "Who is responsible for the proper use of personal protective equipment (PPE)?", opciones: ["The employee", "The general contractor", "A safety manager", "A supervisor"] },
    es: { texto: "¿Quién es responsable del uso adecuado del equipo de protección personal (EPP)?", opciones: ["El empleado", "El contratista general", "Un gerente de seguridad", "Un supervisor"] },
    correcta: 0 },
  { en: { texto: "Which two are acceptable for securing a strap or box to a drywall surface? (Choose two)", opciones: ["A drywall screw directly into drywall", "A nail anchored in a wood stud", "A panhead screw anchored in a wood stud", "A toggle bolt"] },
    es: { texto: "¿Cuáles 2 son aceptables para asegurar un soporte o caja a una superficie de tablaroca (drywall)? (Elige dos)", opciones: ["Un tornillo directo en el drywall", "Un clavo anclado en un poste de madera", "Un tornillo de cabeza plana anclado en un poste de madera", "Un perno de mariposa"] },
    multiple: true, correctas: [2, 3] },
  { en: { texto: "Which fire alarm system component generally depends on circuit polarity reversal to perform its function?", opciones: ["Audible notification appliance", "Ionization detector", "Magnetic door holder", "Waterflow switch"] },
    es: { texto: "¿Qué componente del sistema de alarma generalmente depende de la inversión de polaridad del circuito para funcionar?", opciones: ["Dispositivo de notificación audible", "Detector de ionización", "Retenedor magnético de puerta", "Interruptor de flujo de agua"] },
    correcta: 2 },
  { en: { texto: "What equipment is not prohibited on protective signaling systems by NFPA or UL standards?", opciones: ["Pre-signal systems", "Series indicating appliance circuits", "Unsupervised indicating appliance circuits", "Unsupervised initiating device circuits"] },
    es: { texto: "¿Qué equipo no está prohibido en los sistemas de señalización protectora según las normas NFPA o UL?", opciones: ["Sistemas de pre-señal", "Circuitos indicadores en serie", "Circuitos indicadores sin supervisión", "Circuitos iniciadores sin supervisión"] },
    correcta: 0 },
];
...
function JuegoNICET({ onGanarPuntos }) {
  const [mostrarIntro, setMostrarIntro] = useState(true);
  const [idioma, setIdioma] = useState("es"); // "es" | "en"
  const [respuestas, setRespuestas] = useState({}); // { [i]: number | number[] }
  const [resultado, setResultado] = useState(null);
  const ganadasRef = React.useRef(new Set());
  const falladasRef = React.useRef(new Set());

  const responderUnica = (i, valor) => setRespuestas((prev) => ({ ...prev, [i]: valor }));
  const toggleMultiple = (i, valor) => setRespuestas((prev) => {
    const actual = Array.isArray(prev[i]) ? prev[i] : [];
    const nuevo = actual.includes(valor) ? actual.filter((v) => v !== valor) : [...actual, valor].sort();
    return { ...prev, [i]: nuevo };
  });
  const reiniciarExamen = () => { setRespuestas({}); setResultado(null); };

  const esCorrecta = (p, i) => {
    if (p.multiple) {
      const dadas = Array.isArray(respuestas[i]) ? [...respuestas[i]].sort() : [];
      const correctas = [...p.correctas].sort();
      return dadas.length === correctas.length && dadas.every((v, idx) => v === correctas[idx]);
    }
    return respuestas[i] === p.correcta;
  };

  const enviarExamen = () => {
    const faltantes = PREGUNTAS_NICET.some((p, i) => p.multiple ? !Array.isArray(respuestas[i]) || respuestas[i].length === 0 : respuestas[i] === undefined);
    if (faltantes) {
      setResultado({ ok: null, msg: idioma === "en" ? "Answer all " + PREGUNTAS_NICET.length + " questions before submitting the exam." : "Debes responder las " + PREGUNTAS_NICET.length + " preguntas antes de enviar el examen." });
      return;
    }
    const detalle = PREGUNTAS_NICET.map((p, i) => esCorrecta(p, i));
    const correctas = detalle.filter(Boolean).length;
    const total = PREGUNTAS_NICET.length;
    const porcentaje = Math.round((correctas / total) * 100);
    let puntosGanados = 0;
    detalle.forEach((ok, i) => {
      if (ok && !ganadasRef.current.has(i)) {
        ganadasRef.current.add(i);
        falladasRef.current.delete(i);
        puntosGanados += PUNTOS_POR_PREGUNTA_NICET;
      } else if (!ok && !falladasRef.current.has(i) && !ganadasRef.current.has(i)) {
        falladasRef.current.add(i);
        puntosGanados -= PUNTOS_POR_PREGUNTA_NICET;
      }
    });
    ...
    setResultado({
      ...
      msg: idioma === "en"
        ? (aprobado85 ? `${correctas}/${total} correct (${porcentaje}%). 85% or more — you earned the NICET badge!` : `${correctas}/${total} correct (${porcentaje}%). The NICET badge requires 85% or more. Review the questions marked in red and try again.`)
        : (aprobado85 ? `${correctas}/${total} correctas (${porcentaje}%). ¡85% o más — ganaste la insignia NICET!` : `${correctas}/${total} correctas (${porcentaje}%). La insignia NICET requiere 85% o más. Revisa las preguntas marcadas en rojo y vuelve a intentar.`),
    });
  };
  ...
  if (mostrarIntro) {
    const TEMAS_NICET = [
      ...
      },
      {
        ...
        contenido: (
          <>
            <p>Este examen tiene {PREGUNTAS_NICET.length} preguntas reales de práctica NICET Level 1 (Fire Alarm Systems),
            algunas de selección múltiple ("elige dos" / "elige tres"). Puedes elegir verlas en inglés o en español con el
            selector de idioma arriba del examen.</p>
  ...
  return (
    <Card
      title={idioma === "en" ? "NICET Practice Exam — Fire Alarm Systems" : "Examen de práctica NICET — Fire Alarm Systems"}
      action={
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <div style={{ display: "flex", background: T.graySoft, borderRadius: 8, padding: 3 }}>
            <button onClick={() => setIdioma("es")} style={{ padding: "5px 12px", borderRadius: 6, border: "none", background: idioma === "es" ? T.accent : "transparent", color: idioma === "es" ? "#fff" : T.inkSoft, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>ES</button>
            <button onClick={() => setIdioma("en")} style={{ padding: "5px 12px", borderRadius: 6, border: "none", background: idioma === "en" ? T.accent : "transparent", color: idioma === "en" ? "#fff" : T.inkSoft, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>EN</button>
          </div>
          <Btn small variant="ghost" onClick={() => setMostrarIntro(true)}>{idioma === "en" ? "View the guide again" : "Ver la guía otra vez"}</Btn>
        </div>
      }
    >
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        {PREGUNTAS_NICET.map((p, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              {p.imagen && (
                <div style={{ marginBottom: 12, background: "#fff", border: `1px solid ${T.line}`, borderRadius: 8, padding: 8, display: "inline-block" }}>
                  <img src={p.imagen} alt={`Figura — pregunta ${i + 1}`} style={{ maxWidth: "100%", width: "auto", maxHeight: 320, display: "block" }} />
                </div>
              )}
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {t.opciones.map((op, j) => (
                  <label key={j} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, cursor: "pointer" }}>
                    {p.multiple ? (
                      <input type="checkbox" checked={Array.isArray(respuestas[i]) && respuestas[i].includes(j)} onChange={() => toggleMultiple(i, j)} />
                    ) : (
                      <input type="radio" name={`np${i}`} checked={respuestas[i] === j} onChange={() => responderUnica(i, j)} />
                    )}
                    {op}
                  </label>
                ))}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 18, flexWrap: "wrap" }}>
        <Btn variant="accent" onClick={enviarExamen}>{idioma === "en" ? "Submit exam" : "Enviar examen"}</Btn>
        {resultado && <Btn variant="ghost" onClick={reiniciarExamen}>{idioma === "en" ? "Start over" : "Empezar de nuevo"}</Btn>}
        {resultado && resultado.rangoResultado && (
          <span style={{ fontSize: 13, fontWeight: 800, color: "#fff", background: resultado.rangoResultado.color, padding: "5px 12px", borderRadius: 999 }}>{resultado.rangoResultado.texto}</span>
        )}
        {resultado && <span style={{ fontSize: 14, color: resultado.ok === true ? T.green : resultado.ok === false ? T.red : T.inkSoft, fontWeight: 600 }}>{resultado.msg}</span>}
